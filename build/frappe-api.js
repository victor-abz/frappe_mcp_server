...
export async function getDocument(doctype, name, fields) {
    ...
    const response = await api.get(`/api/resource/${doctype}/${name}${fieldsParam}`);
...
export async function getDocTypeSchema(doctype) {
    const response = await api.get(`/api/method/frappe.desk.form.load.getdoctype?doctype=${encodeURIComponent(doctype)}`);
