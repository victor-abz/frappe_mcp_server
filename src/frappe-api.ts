...
export async function getDocTypeSchema(doctype: string): Promise<any> {
  try {
    ...
    try {
      ...
      console.error(`Checking if DocType document has fields array:`,
        `doctypeDoc=${!!doctypeDoc}`,
        `doctypeDoc.fields=${!!doctypeDoc?.fields}`,
        `Array.isArray=${Array.isArray(doctypeDoc?.fields)}`);
      ...
      return {
        ...
        fields: doctypeDoc.fields.map((field: any) => ({
